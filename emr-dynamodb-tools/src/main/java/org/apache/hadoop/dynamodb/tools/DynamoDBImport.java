...
public class DynamoDBImport extends Configured implements Tool {
    ...
    private void setTableProperties(JobConf jobConf, String tableName, Double writeRatio) {
        ...
        if (description.getBillingModeSummary().getBillingMode().equals(DynamoDBConstants.BILLING_MODE_PROVISIONED)) {
            jobConf.set(DynamoDBConstants.READ_THROUGHPUT, description.getProvisionedThroughput().getReadCapacityUnits()
                    .toString());
            jobConf.set(DynamoDBConstants.WRITE_THROUGHPUT, description.getProvisionedThroughput().getWriteCapacityUnits()
                    .toString());
        } else {
            // Defaulting on 40k for On Demand tables
            jobConf.set(DynamoDBConstants.READ_THROUGHPUT,  DynamoDBConstants.DEFAULT_CAPCITY_FOR_ON_DEMAND);
            jobConf.set(DynamoDBConstants.WRITE_THROUGHPUT, DynamoDBConstants.DEFAULT_CAPCITY_FOR_ON_DEMAND);
        }
