...
public class DynamoDBSplitGenerator {
  ...
  public InputSplit[] generateSplits(int maxClusterMapTasks, int numSegments, JobConf conf, ArrayList<DynamoDBQueryFilter> filterList) {
    log.info("Generating " + numSegments + " segments for " + maxClusterMapTasks + " max mappers");
    ...
    for (int i = 0; i < numMappers; i++) {
      log.info("Assigning " + segmentsPerSplit.get(i).size() + " segments to mapper " + i + ": "
          + segmentsPerSplit.get(i));
      splits[i] = createDynamoDBSplit(getInputPath(conf), approxItemCountPerSplit, i,
          segmentsPerSplit.get(i), numSegments, filterList.get(i));
    }
  ...
  protected DynamoDBSplit createDynamoDBSplit(Path path, long approxItemCount, int splitId,
    ...
    return createDynamoDBSplit(path, approxItemCount, splitId, segments, totalSegments, null);
  }

  protected DynamoDBSplit createDynamoDBSplit(Path path, long approxItemCount, int splitId,
                                              List<Integer> segments, int totalSegments, DynamoDBQueryFilter filter) {
    return new DynamoDBSegmentsSplit(path, approxItemCount, splitId, segments, totalSegments, filter);
  }
}
